...
const API_URL = "https://sponsorlink-backend.up.railway.app/api/bayes/getOrderedUsers/";
...
export default function Explore({isAuthenticated, setIsAuthenticated}) {
  ...
  const handleExplore = useCallback(async (formData) => {
    ...
    const dataToSubmit = {
      ...
      specializedTags: formData.specializedTags.map(tag => ({ name: tag.trim(), slider: 0.5 })), // Example slider value
    };
  ...
  const renderedListings = useMemo(() => {
    return state.displayedItems.map((listing, index) => (
      <ListingCard
        ...
        isSaved={state.savedListings.some(saved => saved.name === listing.name)}
        showApply={true}
        showRemove={false}
        ...
        onShowApplicationModal={() => {
          setState(prev => ({ ...prev, showApplicationModal: true, selectedListing: listing }))
          document.body.style.overflow = 'hidden'
        }}      />
    ));
  }, [state.displayedItems, state.savedListings, state.applications, handleToggleSaveListing]);
  ...
  return (
    <>
      <Navbar isAuthenticated={isAuthenticated} setIsAuthenticated={setIsAuthenticated} hasMenuButton={false} hasSearchbar={state.hasExplored} />
      <div className="main">
        ...
        <>
          {!state.hasExplored ? (
            <div className="explore-container">
              <ExploreForm onSubmit={handleExplore} loading={state.isLoading} />
            </div>
          ) : (
            <>
              <div className="explore-header">
                <div className="explore-dash">
                  <div className="dash-left">
                    <Select initialValue="1">
                      <Select.Option value="1">Sort by Relevance</Select.Option>
                      <Select.Option value="2">Sort by Distance</Select.Option>
                    </Select>
                    <Button onClick={handleNewSearch} scale={0.8} width="20%">Filters</Button>
                  </div>
                ...
                <div className="explore-title">{state.listings.length} Results found</div>
              </div>
              <div className={state.viewMode === 'grid' ? 'grid-auto' : 'list-auto'}>
                {renderedListings}
              </div>
              {state.displayedItems.length < state.listings.length && (
                <div style={{ textAlign: 'center', margin: '20px 0' }}>
                  <Button onClick={() => setState(prev => ({ ...prev, currentPage: prev.currentPage + 1 }))}>
                    Load More
                  </Button>
                </div>
              )}
          ...
          {state.showApplicationModal && (
            <ApplicationModal
              setShowApplicationModal={(value) => setState(prev => ({ ...prev, showApplicationModal: value }))}
              listing={state.selectedListing}
              applicationMessage={state.applicationMessage}
              setApplicationMessage={(value) => setState(prev => ({ ...prev, applicationMessage: value }))}
              handleApplicationSubmit={handleApplicationSubmit}
              handleKeyPress={(e) => {
                if (e.key === 'Enter' && state.applicationMessage.trim() !== '') {
                  e.preventDefault();

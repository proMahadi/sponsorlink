...
export default function Saved() {
  ...
  const { savedListing, removeSavedListing } = useSavedListingContext();
  const [state, setState] = useState(() => ({
    savedListings: savedListing,
    showDetailsPage: false,
    selectedListing: null,
    viewMode: savedListing.length >= 6 ? "grid" : "list",
        ...
        <>
          <div className="flex">
            <div className="saved-header">
              ...
              <span>
                {savedListing.length}{" "}
                {savedListing.length === 1 ? "Listing" : "Listings"}
              </span>
            </div>
            {savedListing.length >= 3 && (
          ...
          <div className="saved-listings">
            <div
              className={state.viewMode === "list" ? "list-auto" : "grid-auto"}
            >
              {savedListing.length > 0 ? (
                savedListing.map((listing, index) => (
                  <ListingCard
                    key={index}
                    listing={listing}
                    isSaved={true}
                    showApply={false}
                    showDetails={true}
                    onToggleSave={() => removeSavedListing(listing.id)}
